    ...
    fun fetchLocalFiles(bucketPath: String) {
        coroutineCompat.launchSingle("fetchLocalFiles") {
            ...
            runCatchSilent({
                ...
                val videos = mediaService.fetchLocalVideos(bucketPath, false)
                val folders = mediaService.fetchLocalFoldersWithVideo(bucketPath)
                LogUtil.logD(TAG, "fetchLocalFiles success: ${folders.size} + ${videos.size}")
                _localFiles.emit(UIState.Content(folders + videos))
            }, {
