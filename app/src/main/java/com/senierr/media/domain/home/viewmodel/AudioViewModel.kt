    ...
    fun fetchLocalFiles(bucketPath: String) {
        coroutineCompat.launchSingle("fetchLocalFiles") {
            ...
            runCatchSilent({
                ...
                val folders = mediaService.fetchLocalFoldersWithAudio(bucketPath)
                LogUtil.logD(TAG, "fetchLocalFiles success: ${folders.size} + ${audios.size}")
                _localFiles.emit(UIState.Content(folders + audios))
            }, {
