...
object EncryptUtil {
    ...
    fun hexString2Bytes(hexString: String?): ByteArray? {
        ...
        val hexBytes = hexStringTemp.uppercase(Locale.ROOT).toCharArray()
     ...
     * @params str 要加密的字符串
     */
    fun encryptRSA(pubKey: ByteArray?, str: String): String? {
        try {
            val keySpec = X509EncodedKeySpec(Base64.decode(pubKey, Base64.DEFAULT))
     ...
     * @params str 要解密的字符串
     */
    fun decryptRSA(pubKey: ByteArray?, encryptData: String): String? {
        try {
            val keySpec = X509EncodedKeySpec(Base64.decode(pubKey, Base64.DEFAULT))
            ...
            return String(cipher.doFinal(Base64.decode(encryptData, Base64.DEFAULT)))
        } catch (e: Exception) {
            e.printStackTrace()
        }
        return null
